  ...
  */
  ...
  if (!ins.isNull()) {
    ...
    for (JsonObject elm : ins) {
      ...
      uint16_t maMax = total > 0 ? elm[F("maxpwr")] | (ablMilliampsMax * length) / total : elm[F("maxpwr")] | ablMilliampsMax; // rough (incorrect?) per strip ABL calculation when no config exists
